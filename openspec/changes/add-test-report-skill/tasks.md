...
- [x] T1.12 将 `reports/` 加入 `.gitignore`

## 验证任务（M1）

- [x] T1-V1 验证 Jest 解析：用 Jest 项目执行「生成测试报告」，产出 Markdown 报告，摘要数据与 Jest 原始输出一致（AC1）
- [x] T1-V2 验证失败用例分析：构造含失败用例的结果，报告失败章节含用例名、文件路径、错误信息（AC2）
- [x] T1-V3 验证解析模式：提供 JUnit XML 文件走解析模式，不触发测试执行即产出报告（AC3）
- [x] T1-V4 验证损坏文件：结果文件损坏时返回明确错误说明，不生成空报告（AC4）
- [x] T1-V5 验证幂等性：同一结果文件多次生成报告，非时间戳内容一致（NFR4）

## M2: P1 扩展（pytest + 覆盖率 + fail_threshold）

- [x] T2.1 实现 pytest 解析器
...
- [x] T2.3 实现 `fail_threshold` 配置项
  - 通过率低于阈值时报告结论附加「未达标」标记（FR2.2）
- [x] T2.4 实现 `coverage` 配置项（auto / on / off）

## 验证任务（M2）

- [x] T2-V1 验证 pytest 解析正确性
- [x] T2-V2 验证覆盖率数据存在时正确呈现，不存在时标注「未获取」（AC5）

## M3: P1 输出格式扩展（HTML + JSON）

- [x] T3.1 实现 HTML 报告生成器
  - 路径：`skills/test-report-skill/generators/html-generator.ts`
  - 同样消费 `TestResultModel`，输出六大章节的 HTML
- [x] T3.2 实现 JSON 伴随产物输出
  - 路径：`skills/test-report-skill/generators/json-generator.ts`
  - 输出结构化的 `TestResultModel` JSON（可选伴随产物）

## 验证任务（M3）

- [x] T3-V1 验证 HTML 输出结构与 Markdown 一致
- [x] T3-V2 验证 JSON 产物可被下游程序解析

## 文档与收尾

- [x] T4.1 编写 Skill 使用文档（含触发意图、配置项、双模式说明）
- [x] T4.2 补充各解析器的边界条件说明（NFR2 降级策略文档化）
